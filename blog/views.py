...
def search_view(request):
    ...
    context = {
        "search": search ,
        'object_list':object_list.order_by('-update_date'),
...
def category_view(request , cats):
    ...
    posts = BlogPost.objects.filter(category=cats,published=True).order_by('-update_date')
...
def list_post_view(request):
    ...
    posts = BlogPost.objects.filter(published=True).order_by('-update_date')
    paginator = Paginator(posts,POSTS_PER_PAGE)
    context = {
    'object_list': paginator.get_page(request.GET.get('page')),
    'category': Category.objects.all(),
